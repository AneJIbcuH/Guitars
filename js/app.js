...
const basketId = document.querySelector('#basket')
// product-info
...
class UI {
    ...
    addBasket (cartItem) {     
        const basketcart = document.createElement('basketcart')
        basketcart.innerHTML = `<div class='product product_basket' data-id="${cartItem.id}">
    ...
    setCartItemBtns (event) {
        if (event.target.closest('.cart-item-remove-btn')) {
            ...
            const basketItems = document.querySelectorAll('.product_basket')
            const basketItem = Object.values(basketItems).find(elem => elem.dataset.id == button.dataset.id)
            basketItem.remove()
